...
$id_list = implode(', ', $id);
print_r("UPDATE node_revision SET node_revision.uid = ? WHERE node_revision.nid IN ( $id_list);" . PHP_EOL);
print_r(PHP_EOL);
$id = array();
// file_managed
...
$id_list = implode(', ', $id);
print_r("UPDATE file_managed SET file_managed.uid = ? WHERE file_managed.fid IN ( $id_list);" . PHP_EOL);
print_r(PHP_EOL);
